...
the data for each sector? You may need to experiment with different Abfunctional forms. Produce a plot
...
mac_funs <- ggplot(carbon) + 
  geom_smooth(aes(Abatement,MC_A, colour=MC_A), method=lm, se=FALSE) +
  geom_smooth(aes(Abatement,MC_B,fill="green"), method=lm, se=FALSE) +
  geom_smooth(aes(Abatement,MC_C, fill="red"), method=lm, se=FALSE) +
  geom_smooth(aes(Abatement,MC_D, fill="orange"), method=lm, se=FALSE) +
  labs(x = "CO2 (Tons)", y = "Cost ($)") +
  scale_color_manual(c("coral", "orchid", "pink", "black"))+
  theme_classic()
